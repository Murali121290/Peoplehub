...
const ShiftApprovalPage: React.FC<ShiftApprovalPageProps> = ({ isOdwOnly = false }) => {
  ...
  const safeManagerShiftRequests = shiftRequests.filter((req: any) => {
    ...
    const isRecursiveReport = reportingSet.ids.has(reqEmpId) || reportingSet.ids.has(reqUserDbId) || (reqRepMgr && reportingSet.names.has(reqRepMgr));
        ...
        <div className="flex items-center gap-3 flex-wrap">
          {!isOdwOnly && (
            <Button
              ...
              className="flex items-center gap-2 !py-1.5 !px-3 !text-xs font-bold shadow-md"
            >
              + Log Shift Entry
            </Button>
          )}
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-neutral-50/60 text-neutral-500 text-xs font-bold uppercase tracking-wider border-b border-neutral-200/80">
                ...
                <th className="p-4">Current Shift / Mode</th>
